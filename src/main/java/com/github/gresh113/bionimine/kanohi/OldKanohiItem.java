...
public class OldKanohiItem extends ArmorItem implements IForgeItem {
	...
	public OldKanohiItem(Properties builder) {
		this(KanohiMaterial.hau, EquipmentSlotType.HEAD, builder);
		kanohiActive = false;
	}

	public OldKanohiItem(EquipmentSlotType slot, Properties builder) {
		this(KanohiMaterial.hau, slot, builder);
		kanohiActive = false;
	}

	public OldKanohiItem(IArmorMaterial materialIn) {
		this(materialIn, EquipmentSlotType.HEAD,
				new Item.Properties().group(BioniMineItemGroup.instance).maxStackSize(1));
		kanohiActive = false;
	}

	public OldKanohiItem(IArmorMaterial materialIn, EquipmentSlotType slot, Properties builder) {
	...
	public void onArmorTick(ItemStack stack, World world, PlayerEntity playerIn) {
		...
		if (!(CurrentKanohiStack == ItemStack.EMPTY || CurrentKanohiStack == null)) {
			OldKanohiItem currentKanohi = (OldKanohiItem) CurrentKanohiStack.getItem();
