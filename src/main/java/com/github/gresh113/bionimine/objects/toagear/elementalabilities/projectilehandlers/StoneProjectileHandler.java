...
import com.github.gresh113.bionimine.client.entities.models.elementalprojectiles.StoneProjectileModel;
...
public class StoneProjectileHandler extends ElementalProjectileHandler {
	private static EntityModel<ElementalProjectileEntity> model = new StoneProjectileModel();
	...
	public void spawnParticles(ElementalProjectileEntity entityIn) {
		...
		double dz = vector.z;
		for (int i = 0; i < 4; ++i) {
			entityIn.world.addParticle(new BlockParticleData(ParticleTypes.BLOCK, Blocks.SMOOTH_STONE.getDefaultState()), entityIn.getPosX() + dx * (double) i / 4.0D, entityIn.getPosY() + dy * (double) i / 4.0D, entityIn.getPosZ() + dz * (double) i / 4.0D, -dx, -dy + 0.2D, -dz);
		}
	}
